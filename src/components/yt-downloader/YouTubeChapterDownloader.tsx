...
const YouTubeChapterDownloader: React.FC = () => {
  ...
  const fetchChapters = async (videoUrl: string) => {
    setLoading(true)
    const response = await fetch(`https://example.com/api/chapters?url=${videoUrl}`)
    ...
    <div className="min-h-screen bg-[#121212] flex flex-col items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-md">
        ...
        <p className="text-gray-400 text-center mb-8">
          Extract and download chapter timestamps from any YouTube video. Perfect for YouTube content creators, video editors, and
          podcasters.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mb-6">
          ...
          >
            <div className="flex items-center justify-center">
              {loading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <Download className="h-5 w-5" />}
              <span className="ml-2">{loading ? "Processing..." : "Download Chapters"}</span>
            </div>
          </button>
        </form>
          ...
          <div className="bg-[#1e1e1e]/80 backdrop-blur-sm border border-[#333] rounded-xl overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.3)] max-w-md w-full md:max-w-2xl lg:max-w-4xl xl:max-w-5xl">
            ...
            <div className="max-h-[300px] overflow-y-auto p-2 scrollbar-none">
              {chapters.map((chapter, index) => (
                ...
                >
                  <div className="text-emerald-400 font-mono mr-3">{chapter.timestamp}</div>
